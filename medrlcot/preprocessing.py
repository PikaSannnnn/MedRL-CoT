...
def xy_split_processing_sft(group, x_func=None, y_func=None):
    ...
    Y_thought = ' '.join([f"{row['sentence']}" for row in Y if row['class'] == 'thought_process'])    # Output with only thought_process and diagnosis
    Y_diag = ' '.join([f"{row['sentence']}" for row in Y if row['class'] == 'diagonsis'])    # Output with only thought_process and diagnosis
    X_prompt = f"""Provide a step-by-step clinical reasoning followed by the diagnosis:\n{X_case}\n
    Thought Process: \n Diagnosis:
    """
    
    Y_prompt = f"""Thought Process: {Y_thought}\n
    Diagnosis: {Y_diag}
    """
    
    return pd.Series({'X': X_prompt, 'Y': Y_prompt})
